...
Graph::Graph(unsigned int mode) {
    ...
    unsigned from, to;
    double weight;
    std::string connectionString = "host=localhost port=5432 dbname=trpo user=postgres password =admin";//Устновка параметров для соединения с бд
    try
    {
        pqxx::connection connectionObject(connectionString.c_str());//Соединение с бд
        pqxx::work worker(connectionObject);
        pqxx::result response = worker.exec("SELECT * FROM cities ORDER BY id");//Запрос на получение всех городов
        num_verts = response.size();
        this->g_num_verts = num_verts;
        //Занесение вершин в список
        ...
        response = worker.exec("SELECT * FROM routes ORDER BY id");//Запрос на получение всех путей
        int i = 0;
        for (size_t k = 0; k < response.size(); k++)
        {
            from = atoi(response[k][1].c_str()) - 1;//Вершина отправления
            to = atoi(response[k][2].c_str()) - 1;//Конечная вершина
            if (mode == 2)//Режим 2 параметров
                weight = atof(response[k][7].c_str());
            else//Параметр расстояния; Параметр стоимости
            ...
            if (this->nodes[i].index == from) {//Если вершина уже рассматривается
                ...
                //Подводим индекс до веришны отправления
                ...
                this->nodes[i].set_neighbors(&this->nodes[neig], weight);//Добавляем вершину
            }

        }

        connectionObject.close();//Закрытие соединения с бд
    }
    ...
    for (unsigned i = 0; i < this->g_num_verts; ++i)
        this->graph_matrix[i][i] = 0;//Обнуление диагонали
...
std::vector<Node*> Graph::A_star(Node* start, Node* goal) {
    ...
    //Инициализация параметров стартовой вершины
    start->g = 0;
...
short Search(std::deque<Node*>& Array2, unsigned int low, unsigned int high)
{
    double p = Array2[(high + low) / 2]->f;//Опорный элемент
